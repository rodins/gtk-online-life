...
class ActorsHistory {
	...
	void playLink(string player, string link) {
		string command = player + link + " &";
		system(command.c_str());
	}
	
	void createPlayItemDialog(PlayItem playItem, 
	                          string sizeFile, string sizeDownload) {
        ...
        switch(response_id) {
			case LINK_RESPONSE_PLAY:
			    ...
			    playLink(playItem.player, playItem.file);
			break;
			case LINK_RESPONSE_DOWNLOAD:
			    ...
			    gtk_clipboard_set_text(clipboardX,
			                           playItem.download.c_str(),
			                           playItem.download.size());
			    playLink(playItem.player, playItem.download);
			break;
	...
	static void linksSizeTask(gpointer arg1, gpointer arg2) {
		...
		if(sizeFile.empty() && sizeDownload.empty()) {
			...
			actorsHistory->createPlayItemDialog(*playItem,
			                                     sizeFile,
			                                     sizeDownload);
		}
		gdk_threads_leave();
	...
	static void constantsTask(gpointer arg, gpointer arg2) {
		...
		if(js.length() > 1000) { // Serial
			...
			if(!playlist_link.empty()) { // Playlists found
				...
				actorsHistory->resultsHistory->setListEpisodesArgs(
				     listEpisodesArgs);
	...
	void onPostExecute(string &page) {
		if(!page.empty()) {
			...
			updateActors();
			// Need to do this if called from dialog
			gtk_toggle_tool_button_set_active(
				               GTK_TOGGLE_TOOL_BUTTON(btnActors), TRUE);
			showActors();
		}else {
		    showActorsError();
		}
